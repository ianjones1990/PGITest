<?php        
	...
	// HEY IAN sup bra all the variables, the html attribute name and $_POST values need to be updated but the underlying code should be gravy,
	//obviouslt the table headers need to be sorted too but that should be it!
	...
	if (isset($_POST['delete']) && isset($_POST['checkbox'])) {
			foreach($_POST['checkbox'] as $del_name){ 
				...
				$sql = "DELETE FROM ports_domains_test WHERE speedThresholdName = '{$del_name}'"; 
	...
	if(isset($_POST['add_row']) && isset($_POST['speedThresholdName']) && isset($_POST['speedThresholds']) && isset($_POST['sampleScenario']) ){
		...
		$sql = "INSERT INTO ports_domains_test (speedThresholdName, speedThresholds, sampleScenario) VALUES ('{$speedThresholdName}','{$speedThresholds}','{$sampleScenario}')";
		sqlsrv_query($conn, $sql);
	}
	...
	$sql = "SELECT speedThresholdName, speedThresholds, sampleScenario FROM ports_domains_test";
	...
	?>
		...
		<!-- Depends on number of input you wish to add -->
